...
function setCharaterPoint(x, y) {
    ...
    left: ${(x * (sizeBlock + 6) + 5).toString()}px;
    top: ${(y * (sizeBlock + 6) + 5).toString()}px;
    height: ${sizeBlock}px;
    width: ${sizeBlock}px;`;
	if (y * (sizeBlock + 6) + 5 > 160) {
		document.getElementById("laberint").scrollTop =
			y * (sizeBlock + 6) + 5 - 160;
	} else {
		document.getElementById("laberint").scrollTop = 0;
	}
	if (x * (sizeBlock + 6) + 5 > 160) {
		document.getElementById("laberint").scrollLeft =
			x * (sizeBlock + 6) + 5 - 160;
	} else {
...
function generate(type = -1) {
	...
	document.getElementById("laberint").style =
		"height: " + widthLab + "px; width: " + widthLab + "px;";
	start(val);
}
...
async function generete_interface() {
	...
	for (let i = 0; i < map.length; i++) {
		...
		for (let j = 0; j < array.length; j++) {
			...
			style += `position: absolute;
			          ...
			          top: ${(i * (sizeBlock + 6)).toString()}px; 
			          left: ${(j * (sizeBlock + 6)).toString()}px; 
			          height: ${sizeBlock.toString()}px; 
			          width: ${sizeBlock.toString()}px;`;
	...
	position: absolute;
	display: inline-block;
	top: ${(map.length * (sizeBlock + 6)).toString()}px;
	left: ${(map.length * (sizeBlock + 6)).toString()}px;
	height: 5px; 
	width: 5px; 
	background-color: transporate; 
	block_image.style = style;`;
}
...
function SetCoinsValue() {
	document.getElementById("CoinsText").innerHTML = `
		<div style='display: block; text-align: left; '>Coins:  ${getCountMoney(coins)} 
		<div style = 'margin: 1px 0px; height: 15px; width: 15px;' id='_coin'></div></div>  
		<div style='display: block; text-align: left; '> Size: ${map.length.toString()}
		</div> 
		<div style='display: block; text-align: left; '>Score:  ${score
			.get(map.length)
			.toString()} 
		</div>`;
}
...
function generateCoins() {
	...
	for (let item of coinsPoints) {
		...
		position: absolute; 
		display: inline-block; 
		top: ${(item[0] * (sizeBlock + 6)).toString()} px; 
		left: ${(item[1] * (sizeBlock + 6)).toString()} px; 
		height: 5px; 
		width: 5px; 
		background-color: transporate; `;
		block_image.style = style;
			...
			go_to_vector.forEach((item) => {
				...
				if (toX != wasX || toY != wasY) {
					if (
						...
						toY < 0 ||
						toY >= n ||
						map[toX][toY].length > 0
					) {
						map[nowX][nowY][item] = 1;
					} else {
						stack.push([[toX, toY], [nowX, nowY], stackElement[2] + 1]);
					}
				}
			});
		}
